...
mod tests {
    ...
    fn test_strip_namespace() {
        assert_eq!(strip_namespace(b"vTPrest"), "vTPrest");
        assert_eq!(
            strip_namespace(b"{http://www.portalfiscal.inf.br/cte}vTPrest"),
            "vTPrest"
        );
        assert_eq!(strip_namespace(b"cte:vTPrest"), "vTPrest");
    }
}
...
pub fn parse_cte_xml(xml_bytes: &[u8]) -> Result<CteInfo, String> {
    ...
    // Stack para rastrear o caminho XML atual (sem namespace)
    let mut path_stack: Vec<String> = Vec::new();
    // Flag para saber se estamos dentro de <emit>
    let mut in_emit = false;
        ...
        match reader.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) | Ok(Event::Empty(ref e)) => {
                ...
                if local_str == "infCte" {
                    for attr in e.attributes().flatten() {
                        if strip_namespace(attr.key.as_ref()) == "Id" {
                            let val = String::from_utf8_lossy(&attr.value).to_string();
                            // Remove prefixo "CTe" se presente
                            chave_cte = Some(val.strip_prefix("CTe").unwrap_or(&val).to_string());
                        }
                    }
                }
            ...
            Ok(Event::Text(ref e)) => {
                ...
                match current_tag {
                    ...
                    "vRec" => {
                        // Fallback se vTPrest não estiver disponível
                        if valor_frete.is_none() {
                            valor_frete = Some(text.clone());
                        }
                    }
